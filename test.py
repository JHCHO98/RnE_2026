...
def csv_to_list_of_dicts(file_path):
    ...
    Returns:
        list: [{title: '...', comment: '...', label: 0}, ...] 형태의 리스트.
    """
    try:
        # 1. CSV 파일 읽기
        df = pd.read_csv(file_path)

        # 2. 'label' 열을 정수형(int)으로 변환 (NaN 값이 있으면 float으로 유지될 수 있으므로, 
        #    가능하다면 먼저 빈 값을 처리하거나, object 타입으로 변환 후 정수 변환을 시도할 수 있습니다.
        #    여기서는 오류 발생 시를 대비해 try-except 블록을 사용합니다.)
        try:
            # NaN을 0 등으로 채우고 정수로 변환하거나, pandas의 IntegerDtype을 사용합니다.
            # 데이터가 깨끗하다고 가정하고 'astype(int)'를 먼저 시도합니다.
            df['label'] = df['label'].astype(int)
        except ValueError:
            print("경고: 'label' 열에 정수로 변환할 수 없는 값이 포함되어 있습니다. 문자열로 유지합니다.")
            
        # 3. DataFrame을 딕셔너리 리스트로 변환 (key: column name, value: cell value)
...
# ==========================================
CONFIG = {
    'model_name': "beomi/KcELECTRA-base-v2022", # 한국어 문장 임베딩 SOTA 모델
    'num_classes': 3,         # 예: 0:보수, 1:진보, 2:중립 (학습용 라벨)
    'hidden_dim': 768,        # BERT 히든 사이즈
...
if __name__ == "__main__":
    ...
    if data_list is None or len(data_list) < 5: # 최소 샘플 수 5개로 가정 (분할 가능하도록)
        ...
        # 이전에 사용했던 더미 데이터를 예비로 사용하려면 이 위치에 배치할 수 있습니다.
        # 예: data_list = dummy_data_fallback 
        exit()
